...
class TestIndex(NebulaTestSuite):
    ...
    def test_tag_index(self):
        ...
        # Lookup
        resp = self.client.execute('LOOKUP ON tag_1 WHERE tag_1.col2 == 18 YIELD _vid, tag_1.col1')
        self.check_resp_succeeded(resp)
        expect = [['101', 'Tom']]
        self.check_out_of_order_result(resp, expect)

        resp = self.client.execute('LOOKUP ON tag_1 WHERE tag_1.col3 > 35.7 YIELD _vid, tag_1.col1')
        self.check_resp_succeeded(resp)
        expect = [['102', 'Jerry'], ['103', 'Bob']]
        self.check_out_of_order_result(resp, expect)

        resp = self.client.execute('LOOKUP ON tag_1 WHERE tag_1.col2 > 18 AND tag_1.col3 < 37.2 YIELD _vid, tag_1.col1')
        self.check_resp_succeeded(resp)
        expect = [['103', 'Bob']]
        self.check_out_of_order_result(resp, expect)
    ...
    def test_edge_index(self):
        ...
        # Lookup
        resp = self.client.execute('LOOKUP ON edge_1 WHERE edge_1.col2 == 22 YIELD edge_1._src, edge_1._dst, edge_1._rank, edge_1.col2')
        self.check_resp_succeeded(resp)
        expect = [['102', '103', 0, 22]]
        self.check_out_of_order_result(resp, expect)

        resp = self.client.execute('LOOKUP ON edge_1 WHERE edge_1.col3 > 43.4 YIELD edge_1._src, edge_1._dst, edge_1._rank, edge_1.col1')
        self.check_resp_succeeded(resp)
        expect = [['102', '103', 0, 'Yellow'], ['101', '102', 0, 'Red']]
        self.check_out_of_order_result(resp, expect)

        resp = self.client.execute('LOOKUP ON edge_1 WHERE edge_1.col2 > 45 AND edge_1.col3 < 44.3 YIELD  edge_1._src, edge_1._dst, edge_1._rank, edge_1.col1')
        self.check_resp_succeeded(resp)
