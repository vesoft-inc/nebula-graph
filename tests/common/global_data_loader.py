...
class GlobalDataLoader(object):
    ...
    def load_nba(self):
        ...
        with open(nba_file, 'r') as data_file:
            resp = self.client.execute(
                'CREATE SPACE IF NOT EXISTS nba(partition_num=10, replica_factor=1, vid_size=30, vid_type = string);USE nba;')
    ...
    def load_student(self):
        resp = self.client.execute(
            'CREATE SPACE IF NOT EXISTS student_space(partition_num=10, replica_factor=1, vid_type = string); USE student_space;')
        assert resp.error_code == ttypes.ErrorCode.SUCCEEDED, resp.error_msg
