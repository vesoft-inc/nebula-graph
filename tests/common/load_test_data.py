...
def load_all_test_data(data_dir, ip, port):
    client_pool = ConnectionPool(ip = ip, port = port, network_timeout=0)
    client = GraphClient(client_pool)
    client.authenticate("user", "password")
    load_nba(data_dir, client)
    load_student(client)
    client_pool.close()


# The whole test will load once, for the only read tests
def load_nba(data_dir, client):
