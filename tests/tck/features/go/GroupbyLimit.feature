Feature: Groupby & limit Sentence
  ...
  Scenario: Syntax test2
    When executing query:
      """
      GO FROM "Marco Belinelli" OVER serve YIELD $$.team.name AS name | GROUP BY 1+1 YIELD COUNT(1), 1+1
      """
  ...
  Scenario: Syntax test4
    When executing query:
      """
      GO FROM "Marco Belinelli" OVER serve YIELD $$.team.name AS name, serve.end_year AS end_year | GROUP BY $-.start_year YIELD COUNT($$.team.name)
      """
    Then a SemanticError should be raised at runtime:
  ...
  Scenario: Syntax test5
    When executing query:
      """
      GO FROM "Marco Belinelli" OVER serve YIELD $$.team.name AS name, serve._dst AS id | GROUP BY $-.start_year YIELD COUNT($-.id)
      """
    Then a SemanticError should be raised at runtime:
  ...
  Scenario: Syntax test6
    When executing query:
      """
      GO FROM "Marco Belinelli" OVER serve YIELD $$.team.name AS name, serve._dst AS id | GROUP BY team YIELD COUNT($-.id), $-.name AS teamName
      """
    Then a SemanticError should be raised at runtime:
  ...
  Scenario: Syntax test7
    When executing query:
      """
      GO FROM "Marco Belinelli" OVER serve YIELD $$.team.name AS name, serve._dst AS id | GROUP BY $-.name YIELD COUNT($-.start_year)
      """
    Then a SemanticError should be raised at runtime:
  ...
  Scenario: Syntax test9
    When executing query:
      """
      GO FROM "Marco Belinelli" OVER serve YIELD $$.team.name AS name, serve._dst AS id | GROUP BY $-.name YIELD COUNT($-.name, $-.id)
      """
    Then a SyntaxError should be raised at runtime:
  ...
  Scenario: Syntax test10
    When executing query:
      """
      GO FROM "Marco Belinelli" OVER serve YIELD $$.team.name AS name, serve._dst AS id |  GROUP BY $-.name, SUM($-.id) YIELD $-.name,  SUM($-.id)
      """
    Then a SemanticError should be raised at runtime:
  ...
  Scenario: Groupby test
    ...
    When executing query:
      """
      GO FROM "Aron Baynes","Tracy McGrady" OVER serve YIELD $$.team.name AS name, serve._dst AS id, serve.start_year AS start, serve.end_year AS end
  ...
  Scenario: Empty input
    When executing query:
      """
      GO FROM "NON EXIST VERTEX ID" OVER serve YIELD $^.player.name as name, serve.start_year as start, $$.team.name as name
  ...
  Scenario: Duplicate column
    When executing query:
      """
      GO FROM "Marco Belinelli" OVER serve YIELD $$.team.name AS name, serve._dst AS id, serve.start_year AS start_year, serve.end_year AS start_year
      | GROUP BY $-.start_year YIELD COUNT($-.id), $-.start_year AS start_year, AVG($-.end_year) as avg
      """
    Then a SemanticError should be raised at runtime:
    When executing query:
