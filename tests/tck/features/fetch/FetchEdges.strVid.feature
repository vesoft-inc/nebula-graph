Feature: Fetch String Vid Edges
  ...
  Scenario: Base fetch prop on an edge
    When executing query:
      ...
      YIELD serve._src, serve._dst, serve._rank, serve.start_year, serve.end_year
      """
  ...
  Scenario: Fetch prop on an edge
    When executing query:
      """
      FETCH PROP ON serve 'Boris Diaw'->'Hawks'
      YIELD serve._src, serve._dst, serve._rank, serve.start_year > 2001, serve.end_year
      """
    Then the result should be, in any order:
  ...
  Scenario: Fetch prop on multiple edges
    When executing query:
      """
      FETCH PROP ON serve 'Boris Diaw'->'Hawks','Boris Diaw'->'Suns'
      YIELD serve._src, serve._dst, serve._rank, serve.start_year, serve.end_year
      """
    Then the result should be, in any order:
      | serve._src   | serve._dst | serve._rank | serve.start_year | serve.end_year |
  ...
  Scenario: Fetch prop works with pipeline
    When executing query:
      """
      GO FROM 'Boris Diaw' OVER serve YIELD serve._src AS src, serve._dst AS dst |
  ...
  Scenario: Fetch prop works with DISTINCT
    When executing query:
      """
      FETCH PROP ON serve 'Boris Diaw'->'Hawks','Boris Diaw'->'Hawks'
    ...
    When executing query:
      ...
      FETCH PROP ON serve $-.src->$-.dst
      YIELD DISTINCT serve._src, serve._dst, serve._rank, serve._dst as dst
      """
    Then the result should be, in any order:
  ...
  Scenario: Fetch prop on not existing vertex
    When executing query:
      """
      FETCH PROP ON serve "Zion Williamson"->"Spurs"
  ...
  Scenario: Fetch prop on exist and not exist edges
    When executing query:
      """
      FETCH PROP ON serve "Zion Williamson"->"Spurs", "Boris Diaw"->"Hawks" YIELD serve._src, serve._dst, serve._rank, serve.start_year
      """
    Then the result should be, in any order:
      | serve._src   | serve._dst | serve._rank | serve.start_year |
  ...
  Scenario: Fetch prop on a edge and return duplicate columns
    When executing query:
      """
      FETCH PROP ON serve "Boris Diaw"->"Spurs" YIELD serve._src, serve._dst, serve._rank, serve.start_year, serve.start_year
      """
    Then the result should be, in any order:
  ...
  Scenario: Fetch and Yield
    When executing query:
      """
      FETCH PROP ON like "Tony Parker"->"Tim Duncan", "Grant Hill" -> "Tracy McGrady" | yield properties($-.edges)
      """
    Then the result should be, in any order:
      | properties($-.edges) |
      | {likeness: 95}        |
    ...
    When executing query:
      """
      FETCH PROP ON like "Tony Parker"->"Tim Duncan", "Grant Hill" -> "Tracy McGrady" | yield endNode($-.edges) AS nodes
      """
    Then the result should be, in any order, with relax comparison:
      | nodes             |
      | ("Tim Duncan")    |
