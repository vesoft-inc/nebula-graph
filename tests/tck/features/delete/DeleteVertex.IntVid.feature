...
Feature: Delete int vid of vertex
  ...
  Scenario: delete int vid vertex
    ...
    When executing query:
      """
      FETCH PROP ON serve hash("Tony Parker")->hash("Spurs") YIELD serve._src, serve._dst, serve._rank, serve.start_year, serve.end_year
      """
    ...
    When executing query:
      """
      FETCH PROP ON serve hash("Grant Hill")->hash("Pistons") YIELD serve._src, serve._dst, serve._rank, serve.start_year, serve.end_year
      """
    Then the result should be, in any order:
      | serve._src          | serve._dst           | serve._rank | serve.start_year | serve.end_year |
    ...
    When executing query:
      """
      FETCH PROP ON player hash("A Loner") YIELD _vid, player.name, player.age
      """
    Then the result should be, in any order:
      | _vid | player.name | player.age |
    # delete with no edge
    ...
    When executing query:
      """
      FETCH PROP ON player hash("Nobody") YIELD _vid, player.name, player.age
      """
    Then the result should be, in any order:
      | _vid | player.name | player.age |
