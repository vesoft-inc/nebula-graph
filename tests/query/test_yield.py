...
class TestYield(NebulaTestSuite):
    ...
    def test_go(self):
        resp = self.client.execute_query('GO FROM "Russell Westbrook" OVER serve, like')
        self.check_resp_succeeded(resp)
        expect_result = [['Thunders', T_NULL],
                         [T_NULL, 'Paul George'],
                         [T_NULL, 'James Harden']]
        self.check_out_of_order_result(resp, expect_result)

        resp = self.client.execute_query('GO FROM "Manu Ginobili" OVER like, teammate REVERSELY YIELD like.likeness, \
            teammate.start_year, $$.player.name')
        self.check_resp_succeeded(resp)
        expect_result = [[95, T_NULL, "Tim Duncan"],
