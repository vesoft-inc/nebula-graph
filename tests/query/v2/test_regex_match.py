...
class TestRegexMatch(NebulaTestSuite):
    ...
    def test_regex_match_with_yield(self):
        ...
        resp = self.execute_query(r'YIELD "Tony Parker" =~ "T\\w+\\s\\w+"')
        self.check_resp_succeeded(resp)
        expected_data = [[True]]
        self.check_result(resp, expected_data)

        resp = self.execute_query(r'YIELD "010-12345" =~ "\\d{3}\\-\\d{3,8}"')
        self.check_resp_succeeded(resp)
        expected_data = [[True]]
        self.check_result(resp, expected_data)
        ...
        resp = self.execute_query(r'YIELD "2019" =~ "\\d+"')
        self.check_resp_succeeded(resp)
        expected_data = [[True]]
        self.check_result(resp, expected_data)
        ...
        resp = self.execute_query(r'YIELD "a good person" =~ "a\\s\\w+"')
        self.check_resp_succeeded(resp)
        expected_data = [[False]]
        self.check_result(resp, expected_data)

        resp = self.execute_query(r'YIELD "Trail Blazers" =~ "\\w+"')
        self.check_resp_succeeded(resp)
        expected_data = [[False]]
        self.check_result(resp, expected_data)

        resp = self.execute_query(r'YIELD "Tony No.1" =~ "\\w+No\\.\\d+"')
        self.check_resp_succeeded(resp)
        expected_data = [[False]]
        self.check_result(resp, expected_data)
