...
class TestBasicTypeCrud(NebulaTestSuite):
    ...
    def test_date_crud(self):
        # create schema
        resp = self.execute('CREATE TAG tag_date(f_date DATE, f_datetime DATETIME)')
        self.check_resp_succeeded(resp)
        ...
        resp = self.execute('CREATE EDGE edge_date(f_date DATE, f_datetime DATETIME)')
        self.check_resp_succeeded(resp)
        ...
        resp = self.execute('INSERT EDGE edge_date(f_date, f_datetime) VALUES "test_src"->"test_dst":(date("2017-03-04"), datetime("2017-03-04T22:30:40"))')
        self.check_resp_succeeded(resp)

        # query
        resp = self.execute_query('FETCH PROP ON tag_date "test"')
        self.check_resp_succeeded(resp)
        result = [["test", CommonTtypes.Date(2017, 3, 4), CommonTtypes.DateTime(2017, 3, 4, 22, 30, 40, 0)]]
        self.check_out_of_order_result(resp, result)

        resp = self.execute_query('FETCH PROP ON edge_date "test_src"->"test_dst"')
        self.check_resp_succeeded(resp)
        result = [["test_src", "test_dst", 0, CommonTtypes.Date(2017, 3, 4), CommonTtypes.DateTime(2017, 3, 4, 22, 30, 40, 0)]]
        self.check_out_of_order_result(resp, result)

        # update
        resp = self.execute_query('UPDATE VERTEX "test" SET tag_date.f_date = Date("2018-03-04"), tag_date.f_datetime = DateTime("2018-03-04T22:30:40") YIELD f_date, f_datetime')
        self.check_resp_succeeded(resp)
        result = [[CommonTtypes.Date(2018, 3, 4), CommonTtypes.DateTime(2018, 3, 4, 22, 30, 40, 0)]]
        self.check_out_of_order_result(resp, result)

        resp = self.execute_query('UPDATE EDGE "test_src"->"test_dst" OF edge_date SET edge_date.f_date = Date("2018-03-04"), edge_date.f_datetime = DateTime("2018-03-04T22:30:40") YIELD f_date, f_datetime')
        self.check_resp_succeeded(resp)
        result = [[CommonTtypes.Date(2018, 3, 4), CommonTtypes.DateTime(2018, 3, 4, 22, 30, 40, 0)]]
        self.check_out_of_order_result(resp, result)
