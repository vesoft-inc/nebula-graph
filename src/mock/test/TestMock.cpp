...
TEST_F(MockServerTest, TestStorage) {
    ...
    storage::cpp2::PropDataByRow propRow;
    std::vector<Value> props;
    props.emplace_back("hello");
    propRow.set_props(std::move(props));
    storage::cpp2::NewTag tag;
    tag.set_tag_id(3);
    tag.set_props(std::move(propRow));
    ...
    auto resp = storageClient->addVertices(space, vertices, false).get();
