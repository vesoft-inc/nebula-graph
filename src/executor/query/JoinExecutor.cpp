...
Status JoinExecutor::checkInputDataSets() {
    ...
    auto lvar = join->left()->outputVar();
    lhsIter_ = ectx_->getVersionedResult(lvar, join->leftVarVersion()).iter();
    DCHECK(!!lhsIter_);
    VLOG(1) << "lhs: " << lvar << " " << lhsIter_->size();
    if (lhsIter_->isGetNeighborsIter() || lhsIter_->isDefaultIter()) {
        std::stringstream ss;
        ss << "Join executor does not support " << lhsIter_->kind();
        return Status::Error(ss.str());
    }
    auto rvar = join->right()->outputVar();
    rhsIter_ = ectx_->getVersionedResult(rvar, join->rightVarVersion()).iter();
    DCHECK(!!rhsIter_);
    VLOG(1) << "rhs: " << rvar << " " << rhsIter_->size();
