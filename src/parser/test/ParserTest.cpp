...
TEST(Parser, InsertVertex) {
    ...
    // Test insert prop unterminated ""
    {
        ...
        std::string query = "INSERT VERTEX person(name, age) "
                            "VALUES \"Tom\":(\"Tom, 30)";
        auto result = parser.parse(query);
        EXPECT_TRUE(result.status().isSyntaxError());
    }
    // Test insert prop unterminated ''
    {
        GQLParser parser;
...
TEST(Parser, Annotation) {
    ...
    }
    {
        ...
        auto result = parser.parse(query);
        EXPECT_TRUE(result.status().isStatementEmpty());
    }
    {
        GQLParser parser;
        std::string query = "# test comment....";
...
TEST(Parser, ReentrantRecoveryFromFailure) {
    GQLParser parser;
    {
        ...
        EXPECT_FALSE(parser.parse(query).ok());
    }
    {
...
TEST(Parser, ErrorMsg) {
    {
        ...
        EXPECT_EQ(error, result.status().toString());
    }
    {
        GQLParser parser;
...
TEST(Parser, Function) {
    {
        GQLParser parser;
        std::string query = "YIELD now()";
        auto result = parser.parse(query);
        EXPECT_TRUE(result.ok()) << result.status();
    }
    {
        GQLParser parser;
        std::string query = "YIELD func(1 + 1)";
        auto result = parser.parse(query);
        EXPECT_TRUE(result.ok()) << result.status();
    }
    // With scope
    {
        GQLParser parser;
        std::string query = "YIELD scope::func(1 + 1) + 1 AS col";
        auto result = parser.parse(query);
        EXPECT_TRUE(result.ok()) << result.status();
    }
}
